...
float get_brightness(float level,vec3 pos) {
...
void main() {
    ...
    float block_brightness = get_brightness(floor(texCoord.x * 16) / 15,fragPos) * BlockFactor;
    float sky_brightness = get_brightness(floor(texCoord.y * 16) / 15,fragPos) * SkyFactor;
    ...
    float lightingFogMultiplier = 1;

    vec3 lightPos = positionL;
    float dist = distance(lightPos, fragPos);
    float radius = radiusL;
    vec3 lightColor = colorL;
    if(dist < radius) {
        if(lightColor.r != -1 || lightColor.g != -1 || lightColor.b != -1) {
            color += (vec3(lightColor * pow(1.0F - dist / radius, 2)) * lightingFogMultiplier);
        }
    }
